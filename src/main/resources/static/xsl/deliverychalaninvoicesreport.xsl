	...
	<xsl:template match="salesorders/salesorder">
		<fo:table-row>
			<fo:table-cell xsl:use-attribute-sets="category.table.td" margin-top="10px">
				...
				<fo:block  font-size="8px" padding-top="10px" padding-bottom="15px" text-align="left">
					<fo:table table-layout="fixed" width="100%" border-collapse="collapse">
						...
						<fo:table-body>
							<fo:table-row>
								...
								<fo:table-cell  text-align="left" font-weight="bold">
									<fo:block><xsl:value-of select="spellword2"/> <xsl:if test="xprimeword2"> and </xsl:if><xsl:value-of select="xprimeword2"/> Taka Only </fo:block>
								</fo:table-cell>
							</fo:table-row>
						</fo:table-body>
					</fo:table>
