...
<xsl:stylesheet 
	...
	<xsl:template match="matrixreport">
				...
				<fo:static-content flow-name="header-first">
					...
					<fo:block-container width="100%" border-bottom ="1px solid #000000" >
						...
						<fo:block text-align="left" font-size="7px" font-weight="bold"  margin-top="2px" padding-bottom="5px">
							Requisition Date: <xsl:value-of select="fromDate"/>
						</fo:block>
					</fo:block-container>
				</fo:static-content>
	...
	<xsl:template match="datas/data">
			...
			<fo:table-cell>
				<fo:block>
					<fo:table table-layout="fixed" width="100%" border-collapse="collapse">
						...
						<fo:table-header>
							<fo:table-row>
								<fo:table-cell xsl:use-attribute-sets="client.table.th" number-rows-spanned="3"><fo:block>Item / Branch</fo:block></fo:table-cell>
								<xsl:for-each select="columns/column">
									<fo:table-cell xsl:use-attribute-sets="client.table.th"><fo:block text-align="right"><xsl:value-of select="xitem"/></fo:block></fo:table-cell>
								</xsl:for-each>
							</fo:table-row>
							<fo:table-row>
								<xsl:for-each select="columns/column">
									<fo:table-cell xsl:use-attribute-sets="client.table.th"><fo:block text-align="right"><xsl:value-of select="xdesc"/></fo:block></fo:table-cell>
								</xsl:for-each>
							</fo:table-row>
							<fo:table-row>
								<xsl:for-each select="columns/column">
									<fo:table-cell xsl:use-attribute-sets="client.table.th"><fo:block text-align="right"><xsl:value-of select="xunitpur"/></fo:block></fo:table-cell>
								</xsl:for-each>
							</fo:table-row>
						</fo:table-header>
						...
						<fo:table-body>
							...
							<fo:table-row>
								<fo:table-cell xsl:use-attribute-sets="client.table.th"><fo:block>Total</fo:block></fo:table-cell>
								<xsl:for-each select="totals/total">
									<fo:table-cell xsl:use-attribute-sets="client.table.th"><fo:block text-align="right"><xsl:value-of select="xqtyord"/></fo:block></fo:table-cell>
								</xsl:for-each>
							</fo:table-row>
						</fo:table-body>
					</fo:table>
		...
		<fo:table-row xsl:use-attribute-sets="client.table.tr.divider">
			<fo:table-cell>
				<fo:block></fo:block>
			</fo:table-cell>
		</fo:table-row>
	...
	<xsl:template match="rows/row">
		<fo:table-row>
			<fo:table-cell xsl:use-attribute-sets="client.table.td"><fo:block><xsl:value-of select="zorg"/></fo:block></fo:table-cell>
			<xsl:for-each select="items/item">
				<fo:table-cell xsl:use-attribute-sets="client.table.td"><fo:block text-align="right"><xsl:value-of select="xqtyord"/></fo:block></fo:table-cell>
			</xsl:for-each>
		</fo:table-row>
	</xsl:template>
	...
	<xsl:template match="batches/batch">
		<fo:table-row>
			<fo:table-cell xsl:use-attribute-sets="category.table.td">
				...
				<fo:block font-size="8px" margin-top="20px" margin-bottom="5px" text-align="left">
					<fo:table table-layout="fixed" width="100%" border-collapse="collapse" >
						...
						<fo:table-body>
							<fo:table-row>
								<fo:table-cell>
									<fo:block font-weight="bold">Batch</fo:block>
								</fo:table-cell>
								<fo:table-cell>
									<fo:block>:</fo:block>
								</fo:table-cell>
								<fo:table-cell>
									<fo:block font-weight="bold"><xsl:value-of select="xbatch"/></fo:block>
								</fo:table-cell>
							</fo:table-row>
							<fo:table-row>
								<fo:table-cell>
							...
							<fo:table-row>
								<fo:table-cell>
									<fo:block>Production Quantity</fo:block>
								</fo:table-cell>
								<fo:table-cell>
									<fo:block>:</fo:block>
								</fo:table-cell>
							...
							<fo:table-row>
								<fo:table-cell>
									<fo:block>Quantity Completed</fo:block>
								</fo:table-cell>
								<fo:table-cell>
									<fo:block>:</fo:block>
								</fo:table-cell>
