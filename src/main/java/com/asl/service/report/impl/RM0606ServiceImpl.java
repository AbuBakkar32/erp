 ...
 * @since Dec 27, 2020
 */
@Service("RM0606Service")
public class RM0606ServiceImpl extends AbstractReportService {
	...
	private List<FormFieldBuilder> generateFields() {
		...
		List<DropdownOption> Store = new ArrayList<>();
		//Store.add(new DropdownOption("", "-- Select --"));
		storeList.stream().forEach(x -> Store.add(new DropdownOption(x.getXcode(), x.getXcode() + "-" + x.getXlong())));
		...
		// Store
		fieldsList.add(FormFieldBuilder.generateDropdownField(7, "Warehouse", Store, "", false));
