...
@Controller
@RequestMapping("/supopening")
public class ACCSupplierOpeningController extends ASLAbstractController{
	...
	public String loadCusAdjusmentPage(@PathVariable String xvoucher, Model model) {
		...
		data.setXtypetrn(TransactionCodeType.ACCOUNT_OBAP.getCode());
	...
	private Arhed getDefaultArhed() {
		...
		arhed.setXtrntype(TransactionCodeType.ACCOUNT_OBAP.getCode());
	...
	public @ResponseBody Map<String, Object> save(Arhed arhed, BindingResult bindingResult){
		...
		if (arhed.getXtyperec().equals("Advance")){
			arhed.setXsign(1);
		}
		...
		if(StringUtils.isNotBlank(arhed.getXvoucher())) {
			...
			responseHelper.setSuccessStatusAndMessage("Supplier Opening updated successfully");
			responseHelper.setRedirectUrl("/supopening/" + arhed.getXvoucher());
			return responseHelper.getResponse();
		}
		...
		responseHelper.setSuccessStatusAndMessage("Supplier Opening created successfully");
		responseHelper.setRedirectUrl("/supopening/" + arhed.getXvoucher());
		return responseHelper.getResponse();
	}
	...
	public @ResponseBody Map<String, Object> delete(@PathVariable String xvoucher, Model model){
		...
		if(voucher == null) {
			responseHelper.setErrorStatusAndMessage("Supplier Opening not found in this system");
			return responseHelper.getResponse();
		}

		long count = arhedService.deleteVoucher(xvoucher);
		if(count == 0) {
			responseHelper.setErrorStatusAndMessage("Can't delete Supplier Opening");
			return responseHelper.getResponse();
		}

		responseHelper.setSuccessStatusAndMessage("Supplier Opening deleted successfully");
		responseHelper.setRedirectUrl("/supopening");
		return responseHelper.getResponse();
	}
	...
	public @ResponseBody Map<String, Object> confirmMoneyReceipt(@PathVariable String xvoucher){
		...
		if(count == 0) {
			responseHelper.setErrorStatusAndMessage("Can't confirm Supplier Opening");
			return responseHelper.getResponse();
		}

		responseHelper.setSuccessStatusAndMessage("Supplier Opening confirmed successfully");
		responseHelper.setRedirectUrl("/supopening/" + voucher.getXvoucher());
		return responseHelper.getResponse();
	}
